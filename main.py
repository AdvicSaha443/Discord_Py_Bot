...
async def music_help(ctx):
  ...
  ("?resume", "Resumes The Song", False),
  ("?stop", "Stops The Song", False),
...
async def kick(ctx, member: discord.Member, *, reason=None):
  ...
  if str(ctx.author) in mods:
    if str(ctx.author) != member:
      ...
      await ctx.channel.send(f"{ctx.author.mention} You Can't Kick YourSelf!")
  else:
...
@bot.command()
async def temp_mute(ctx, member: discord.Member, time: int):
  mods = []
  mod_role = discord.utils.get(ctx.guild.roles, name="Bot-Mod")
...
async def invest(ctx, amount = None):
  ...
  if random_num > 0:
    ...
    await update_bank(ctx.author, total_amount, "wallet")
    mbed = discord.Embed(
...
async def adventure(ctx, amount):
  ...
  if adv is None:
    ...
    if adv_value == True:
      ...
      if amount > 500000:
        await ctx.send("?adventure Doesn't Support Coins More Than 500k Use ?adv To Invest Till 10M!")
      else:
        adv_db.update_one({"id":ctx.author.id}, {"$set":{"adv":True}})
        adv_db.update_one({"id":ctx.author.id}, {"$set":{"mLO":amount}})

        await ctx.send(f"{ctx.author.mention}, Going On An Adventure With {amount} SB Coins For 30 Min!")
        await update_bank(ctx.author, -1*amount)
...
async def rollMLO(ctx):
  ...
  if mlo == 0:
    ...
    await update_bank(ctx.author, mlo, "wallet")
...
async def buy_role(ctx, role: discord.Role):
  ...
  MVP = get(ctx.guild.roles, name="MVP")
  MVP1 = get(ctx.guild.roles, name="MVP+")
  MVP2 = get(ctx.guild.roles, name="MVP++")
  SuperMVP = get(ctx.guild.roles, name="Super-MVP+")
  SuperMVP1 = get(ctx.guild.roles, name="Super-MVP++")
  ...
  if role == SuperVIP3:
    ...
    if role == MVP:
      ...
      if amount<bal[0]:
        await update_bank(ctx.author, -1*amount)
        await ctx.author.add_roles(role)
        await ctx.send("You Just Bought MVP Role!")
        return
      else:
        await ctx.send("You Don't Have Enough Coins")
  ...
  if role == SuperMVP:
    ...
    if amount<bal[0]:
      await update_bank(ctx.author, -1*amount)
      await ctx.author.add_roles(role)
      await ctx.send("You Just Bought Super-MVP+ Role!")
      return
    else:
      await ctx.send("You Don't Have Enough Coins")
...
async def on_member_join(member):
  ...
  await member.add_roles(role)
  await channel.send(f"Heya {member.mention}!" + " Welcome To **Kingdom** For More Information Go To <#875000973221838849>", embed = mbed)
